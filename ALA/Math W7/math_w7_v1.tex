...
\begin{document}
	...
	und Monotonie, Beschränktheit und Konvergenz.\\
	\\
	\\
	...
	\[
		\begin{cases}
			\begin{array}{lllll}
				n=1 & : & \frac{1}{3}-\frac{1}{2} & =-\frac{1}{6}	& \Rightarrow \text{Infimum}\\
				n\rightarrow+\infty & : & \frac{1}{3}-\frac{1}{2n} & \rightarrow\frac{1}{3}^\text{+} & \Rightarrow \text{Supremum}
			\end{array}
		\end{cases}
	\]
	und der Grenzwert, gegen den die Folge konvergiert, ist dementsprechend $\frac{1}{3}$.\\
	\\
	Da der abgezogene Betrag mit jeder Stufe geringer wird, ist die Folge streng monoton steigend.
	...
	\\
	\\
	Da sich die Folge $2^n$ annähert, ist sie monoton steigend.
	Ebenso wie $2^n$ist die Folge $(a_n)$ nicht beschränkt.
	Der Grenzwert ist dementsprechend $+\infty^\text{-}$
	\section*{Aufgabe 3}
	\label{sec:a3}
	...
	\\
	\\
	($a_n$) ist beschränkt $\Rightarrow$ Sie hat ein Supremum und ein Infimum.\\
	Für $b_n$ sei nun das Supremum $\operatorname{sup}(b_n)=\operatorname{sup}\lbrace (a_m)|m\ge n \rbrace$	(Für genügend hohe $n$ muuss auch das $m$ genügend groß gewählt werden).\\
	Somit kann man sagen: $\operatorname{sup}(b_n)=\operatorname{sup}(a_n)|\operatorname{sup}(a_n)\text{ liegt nicht in }(b_n)$.\\
	\-~~~~$\Rightarrow (b_n)$ ist nicht beschränkt.
	\begin{proof}
	$(b_n)$ ist monoton:\\
	\[
		\operatorname{sup}(b_n)=\operatorname{sup}\lbrace (a_m)|m\ge n \rbrace\Rightarrow\operatorname{sup}(b_n)=\operatorname{sup}\left(\underset{m\ge n}{\bigcup}a_m\right)
	\]
	Daraus folgt, dass der minimale Wert der Menge immer kleiner wird.
	\[
		\operatorname{sup}(b_n)=\operatorname{sup}\lbrace(a_m)|m\ge n\rbrace:=A~~~~\wedge~~~~\operatorname{sup}(b_{n+1})=\operatorname{sup}\lbrace (a_m)|m\ge n+1 \rbrace:=B
	\]
	\[\Rightarrow B\underline{\subset}A\]
	Folglich muss $b_n$ monoton steigend sein, da $b_n\le b_{n+1}$ wegen $B\underline{\subset}A$.
	Laut Satz 4.23 ist $b_n$ somit beschränkt und monoton $\Rightarrow$ $b_n$ konvergiert auch.
	\end{proof}
	\subsection*{(b)}
	\label{ssec:a5.b}
	...
	\\
	\textit{Cauchy-Folge:}\\
	\[a_n|\forall \epsilon>0:\exists n_0\in\mathbb{N}:\forall n,m\ge n_0:|a_n-a_m|<\epsilon|a_m:=a_n\text{ für den Wert }n\]
	\textsc{Konvergenz:}
	\[a_n|\forall\epsilon>0:\exists n_0\in\mathbb{N}:n\ge n_0:|a_n-a|<\epsilon|a:=\text{ Zahl, gegen die }a\text{ konvergiert}\]
	Jede Folge, die sowohl monoton als auch beschränkt ist, konvergiert.\\
	\\
	Sei $c_n$ eine Cauchy-Folge.\\
	Sei $\epsilon=1~~~\wedge~~~n_0\in\mathbb{N}~~~\wedge~~~|c_n-c_m|<1~\forall n,m\ge n_0$.\\
	Sei $m=n_0 \Rightarrow |c_n-c_{n_0}| < 1\Rightarrow |c_n| < |c_{n_0}|+1~\forall n\ge n_0$.\\
	\[\Rightarrow |c_n|\le \operatorname{sup}\lbrace |c_1|,|c_2|,\cdots,|c_n|,1+|c_n| \rbrace\]
	Daraus folgt, dass sie beschränkt ist, da sie auch endlich ist.\\
	Wenn $(c_n)$ nun auch monoton wachsend ist, konvergiert $(c_n)$.\\
	Laut Satz von Bolzano-Weierstrauß besitzt jede beschränkte Folge eine konvergente Teilfolge.
	Durch $ |c_n-c| \underline{\subset} |c_n-c_{n_i} |+|c_{n_i} -c|$ ergibt sich, dass a der Grenzwert von ganz $(c_n)$ sein muss.\\
	\textit{((Dreiecks-Ungleichung) $c_{n_i} :=$ Teilfolge mit $i\in\mathbb{N}$, die gegen a konvergiert.)}
\end{document}