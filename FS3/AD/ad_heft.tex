...
$sum(s)$ und $max(s)$ haben jeweils eine lineare Laufzeit von $n$.
Damit werden insgesamt $2\cdot n\cdot\operatorname{log}n$ Schritte zur vollständigen Berechnung benötigt, die obere Schranke liegt also in $\mathcal{O}(n\cdot\operatorname{log}n)$.
...
%	Bearbeitung	%
%---------------%
\texttt{2-SAT} kann nur Implikationen der Form $a \Rightarrow b$ ausdrücken.
Ebenso kann jedweder zweistellige Ausdruck (Beispiel: $a \lor b$ wird zu $\lnot a \Rightarrow b \land \lnot b \Rightarrow a$) als Paar von Implikationen dargestellt werden.
Wenn also $a$ wahr ist, muss auch 
