...
\\
Dies wird so nicht funktionieren, da der Primärschlüssel, welcher nicht \textsc{NULL} sein darf, fehlt und somit \textsc{NULL} ist.
...
\textbf{SELECT} RO.OID\\
\textbf{FROM} Rennort RO\\
\textbf{WHERE} Ro.Name = $\wert{Großbritannien~GP}$,\\
5);
...
Und er rechnet mit sehr großen Tupeln.
Und er rechnet durchgängig mit großen Zahlen.
\\
\\
