...
Dieser Bericht befasst sich mit dem Praktikum Agile Softwareentwicklung\footnote{STiNE-Veranstaltungsnummer 64-142} (im Folgenden \string"ASE")
 ...
 , absolviert im zweiten Fachsemester %(im Folgenden "FS2")
des Studiengangs Software-System-Entwicklung %(im Folgenden \string"SSE")
.\\
Dieser Bericht ist geschrieben aus Sicht der Teilnehmer des Teilkurses 3 (ASE-SEP3)
, in welchem das Zuul-Ausgangssystem, welches von den Praktikumsbetreuern bereit gestellt wurde, in ein \textit{Point \& Click\footnote{Ein Programm, bei dem Aktionen durch ein Anvisieren mit einem Zeiger (meist die Maus) und das darauffolgende Klicken hervorgerufen werden.}}-ähnliches \textit{Adventure-Game\footnote{Ein Spiel, bei dem die Erkundung und das Lösen von Rätseln im Vordergrund stehen.}} weiterentwickelt wurde, welches nun den Titel \textit{KUUL} trägt.\\
...
\#Lösungen
\\
Doch was ist \textit{KUUL} eigentlich?

%
%-----------------------------------------------
%

\subsection{Spielprinzip und Hintergrund}
...
\\
Zuerst einmal landet der Spieler in einer Welt und muss sich dort zurecht finden. Die Welt von Kuul besteht aus mehreren Räumen, durch die und in denen man sich bewegen kann. In diesen Räumen gibt es Objekte, mit denen der Spieler interagieren kann und so muss er nach und nach die Welt erkunden und einige Rätsel lösen, um herauszufinden, wie er das Spiel gewinnen kann.
\\
...
Dies wurde vom Entwicklerteam aufgegriffen, sodass die Umgebung \textit{Universität} beibehalten, während die Anzahl der Räume mehr als verdoppelt wurde.
...
Dies führt dazu, dass sie irgendwann keine Energie mehr zurückgeben, oder sogar welche abziehen.
...
Ist sie auf 0, wird sie inkrementiert; ist sie auf 1, so wird der Effekt des Lebenskommandos\footnote{interne Umsetzung der Energieverwaltung, für die Verwendung der Gegenstände nicht weiter von Bedeutung. Für Kommandos s. \ref{ssec:main_commands}} dekrementiert und der \texttt{\_verfallszeitpunkt} wird wieder auf 0 gesetzt.
...
\end{lstlisting}
Der hier betrachtete Quellcode (Code 2) ist aus dem in \textit{KUUL} verwendeten XML-Parser entnommen.
Er sucht sich den Tag \texttt{beenden} in Code 1 und packt den zum Tag gehörigen Gegenstand zusammen mit der Nachricht, die sich in dem Tag befindet, in eine Liste. Diese Liste kann nun an anderer Stelle durchlaufen werden, um die Commands\footnote{s. Kapitel \ref{ssec:main_commands}} festzulegen.\\
...
\\
Weitere Möglichkeiten, das Spiel zu beenden, sind, wenn deine Lebenspunkte auf 0 sinken, oder eine festgelegte Zeit abläuft.
...
Jeder Gegenstand hält damit seine eigene Liste an Commands, die abgefragt werden können, wenn mit dem Gegenstand interagiert wird. So wird sichergestellt, dass eine Aktion nur ausgeführt wird, wenn der entsprechende Gegenstand auch benutzt wurde.\\
